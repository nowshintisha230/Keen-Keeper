...
const Friends = async () => {
  ...
  if (!data || data.length === 0) {
    return (
      <p className="text-center mt-10 text-gray-500">No friends found</p>
    );
  }
  ...
  return (
    <div>
      ...
      <div className="m-5 grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <button className="btn p-10 flex flex-col items-center">
          <span className="text-xl font-bold">{onTrack}</span>
          <p className="text-gray-500 text-sm">On Track</p>
        </button>

        <button className="btn p-10 flex flex-col items-center">
          <span className="text-xl font-bold">{needAttention}</span>
          <p className="text-gray-500 text-sm">Need Attention</p>
        </button>

        <button className="btn p-10 flex flex-col items-center">
          <span className="text-xl font-bold">{interactionThisMonth}</span>
            ...
            <div className="flex flex-col items-center gap-4 bg-gray-200 rounded-3xl shadow-xl p-6 w-full overflow-hidden hover:scale-105 transition">
              ...
              <div className="flex flex-wrap justify-center gap-1">
                {friend.tags?.map((tag, index) => (
                  <span
                    key={index}
                    className="font-bold text-xs px-2 py-1 rounded-full bg-blue-200"
                  >
                    {tag}
                  </span>
                ))}
